...
func TestOpenStackClusterV1Beta2ValidateCreate(t *testing.T) {
	tests := []struct {
		name     string
		template *infrav1beta2.OpenStackCluster
		wantErr  bool
	}{
		{
		...
		{
			name: "OpenStackCluster.Spec.ManagedSecurityGroups.ControlPlaneNodesSecurityGroupRules with mutually exclusive fields on create",
			template: &infrav1beta2.OpenStackCluster{
				Spec: infrav1beta2.OpenStackClusterSpec{
					IdentityRef: infrav1beta2.OpenStackIdentityReference{
						Name:      "foobar",
		...
		{
			name: "OpenStackCluster.Spec.ManagedSecurityGroups.WorkerNodesSecurityGroupRules with mutually exclusive fields on create",
			template: &infrav1beta2.OpenStackCluster{
				Spec: infrav1beta2.OpenStackClusterSpec{
					IdentityRef: infrav1beta2.OpenStackIdentityReference{
						Name:      "foobar",
						...
						WorkerNodesSecurityGroupRules: []infrav1beta2.SecurityGroupRuleSpec{
							{
								...
								Protocol:            ptr.To("tcp"),
								RemoteManagedGroups: []infrav1beta2.ManagedSecurityGroupName{"controlplane"},
								RemoteIPPrefix:      ptr.To("10.0.0.0/24"),
							},
						},
					},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			webhook := &openStackClusterWebhookV1Beta2{}
			...
			g.Expect(warn).To(gomega.BeEmpty())
		})
	}
}

func TestOpenStackClusterV1Beta2ValidateUpdate(t *testing.T) {
	tests := []struct {
		name        string
		oldTemplate *infrav1beta2.OpenStackCluster
		newTemplate *infrav1beta2.OpenStackCluster
		wantErr     bool
	}{
		{
