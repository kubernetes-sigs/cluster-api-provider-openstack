...
func WithInitLocalAPIEndpointAndPort(endpoint string, port int) InitConfigurationOption {
	return func(j *kubeadmv1beta1.InitConfiguration) {
		j.LocalAPIEndpoint.AdvertiseAddress = endpoint
		j.LocalAPIEndpoint.BindPort = int32(port)
	}
}
...
// WithAPIServerExtraVolumes sets the kube-apiserver's extra arguments in the ClusterConfiguration.
func WithAPIServerExtraVolumes(args []kubeadmv1beta1.HostPathMount) ClusterConfigurationOption {
	return func(c *kubeadmv1beta1.ClusterConfiguration) {
		c.APIServer.ExtraVolumes = append(c.APIServer.ExtraVolumes, args...)
	}
}
...
// WithControllerManagerExtraVolumes sets the kube-apiserver's extra arguments in the ClusterConfiguration.
func WithControllerManagerExtraVolumes(args []kubeadmv1beta1.HostPathMount) ClusterConfigurationOption {
	return func(c *kubeadmv1beta1.ClusterConfiguration) {
		c.ControllerManager.ExtraVolumes = append(c.ControllerManager.ExtraVolumes, args...)
	}
}
...
func WithClusterNetworkFromClusterNetworkingConfig(networkingConfig *clusterv1.ClusterNetworkingConfig) ClusterConfigurationOption {
	return func(c *kubeadmv1beta1.ClusterConfiguration) {
		...
		c.Networking.ServiceSubnet = networkingConfig.Services.CIDRBlocks[0]
	}
}
...
// WithTLSBootstrapToken sets the tlsBootstrapToken on the Discovery field to the passed in value.
func WithTLSBootstrapToken(tlsBootstrapToken string) JoinConfigurationOption {
	return func(j *kubeadmv1beta1.JoinConfiguration) {
		j.Discovery.TLSBootstrapToken = tlsBootstrapToken
	}
}
