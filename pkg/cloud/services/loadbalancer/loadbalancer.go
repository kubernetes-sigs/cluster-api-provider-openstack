...
// If PrimarySubnet is specified in the cluster spec, that subnet is resolved
// and its ID is returned. Otherwise the first subnet in the cluster status is used.
func (s *Service) getPrimarySubnetID(openStackCluster *infrav1.OpenStackCluster) (string, error) {
	...
	subnet, err := s.networkingService.GetSubnetByParam(openStackCluster.Spec.PrimarySubnet)
	if err != nil {
