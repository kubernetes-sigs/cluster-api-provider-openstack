...
func (s *Service) constructNetworks(openStackCluster *infrav1.OpenStackCluster, instanceSpec *InstanceSpec) ([]infrav1.Network, error) {
	...
	for i, port := range instanceSpec.Ports {
		pOpts := &instanceSpec.Ports[i]
		// No Trunk field specified for the port, inherit openStackMachine.Spec.Trunk.
		if pOpts.Trunk == nil {
			pOpts.Trunk = &instanceSpec.Trunk
		}
		if *pOpts.Trunk {
			trunkRequired = true
		}
...
func (s *Service) ReconcileInstance(eventObject runtime.Object, openStackCluster *infrav1.OpenStackCluster, instanceSpec *InstanceSpec, clusterName string) (*InstanceStatus, error) {
	return s.reconcileInstanceImpl(eventObject, openStackCluster, instanceSpec, clusterName, retryIntervalInstanceStatus)
}

func (s *Service) reconcileInstanceImpl(eventObject runtime.Object, openStackCluster *infrav1.OpenStackCluster, instanceSpec *InstanceSpec, clusterName string, retryInterval time.Duration) (*InstanceStatus, error) {
	instanceStatus, err := s.GetInstanceStatusByName(eventObject, instanceSpec.Name)
	if err != nil {
