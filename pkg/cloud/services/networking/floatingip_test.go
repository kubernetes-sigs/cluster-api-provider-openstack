...
import (
	...
	"sigs.k8s.io/cluster-api-provider-openstack/pkg/cloud/services/networking/mock_client"
)
...
func Test_GetOrCreateFloatingIP(t *testing.T) {
	...
	tests := []struct {
		name   string
		ip     string
		expect func(m *mock_client.MockNetworkClientMockRecorder)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			mockClient := mock_client.NewMockNetworkClient(mockCtrl)
