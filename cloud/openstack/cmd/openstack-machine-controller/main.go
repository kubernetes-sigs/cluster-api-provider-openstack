...
func main() {
	...
	actuator, err := openstack.NewMachineActuator(client.ClusterV1alpha1().Machines(corev1.NamespaceDefault))
	if err != nil {
		glog.Fatalf("Could not create Openstack machine actuator: %v", err)
	}
