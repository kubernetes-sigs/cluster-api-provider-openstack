...
var _ = Describe("OpenStackCluster controller", func() {
	...
	It("should fail when namespace is denied access to ClusterIdentity and set OpenStackCredentialsReadyCondition to False", func() {
		...
		// Verify OpenStackCredentialsReadyCondition is set to False
		Expect(v1beta1conditions.IsFalse(updatedCluster, infrav1.OpenStackCredentialsAvailableCondition)).To(BeTrue())
		condition := v1beta1conditions.Get(updatedCluster, infrav1.OpenStackCredentialsAvailableCondition)
		Expect(condition).ToNot(BeNil())
		Expect(condition.Reason).To(Equal(infrav1.OpenStackCredentialsFailedReason))
	...
	It("should set OpenStackCredentialsReadyCondition to False when credentials secret is missing", func() {
