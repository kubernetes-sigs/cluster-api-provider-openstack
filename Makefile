...
create-cluster: $(CLUSTERCTL) $(KUSTOMIZE) $(ENVSUBST) ## Create a development Kubernetes cluster on OpenStack in a KIND management cluster.
	...
	$(CLUSTERCTL) init --core cluster-api:v0.3.3 --bootstrap kubeadm:v0.3.3 --control-plane kubeadm:v0.3.3
	...
	# Deploy calico
	cat hack/ci/e2e-conformance/calico.yaml | sed "s/veth_mtu:.*/veth_mtu: \"1400\"/g" | \
