...
create-cluster: $(KUSTOMIZE) $(ENVSUBST) ## Create a development Kubernetes cluster on OpenStack in a KIND management cluster.
	...
	mkdir -p ./out/infrastructure-openstack/0.4.0
	echo "providers:" > ./out/clusterctl.yaml
	echo "- name: openstack" >> ./out/clusterctl.yaml
	echo "  url: $(PWD)/out/infrastructure-openstack/0.4.0/infrastructure-components.yaml" >> ./out/clusterctl.yaml
	echo "  type: InfrastructureProvider" >> ./out/clusterctl.yaml

	echo "releaseSeries:" > ./out/infrastructure-openstack/0.4.0/metadata.yaml
	echo "- major: 0" >> ./out/infrastructure-openstack/0.4.0/metadata.yaml
	echo "  minor: 4" >> ./out/infrastructure-openstack/0.4.0/metadata.yaml
	echo "  contract: v1alpha3" >> ./out/infrastructure-openstack/0.4.0/metadata.yaml
	...
	MANIFEST_IMG=$(CONTROLLER_IMG)-$(ARCH) MANIFEST_TAG=$(TAG) $(MAKE) set-manifest-image
	$(KUSTOMIZE) build config > ./out/infrastructure-openstack/0.4.0/infrastructure-components.yaml
